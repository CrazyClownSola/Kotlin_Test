...
class MediaPlayerCompat {
    static String getName(IMediaPlayer mp) {
        if (mp == null) {
            return "null";
    ...
    private static IjkMediaPlayer getIjkMediaPlayer(IMediaPlayer mp) {
    ...
    static void selectTrack(IMediaPlayer mp, int stream) {
    ...
    static void deselectTrack(IMediaPlayer mp, int stream) {
        IjkMediaPlayer ijkMediaPlayer = getIjkMediaPlayer(mp);
        if (ijkMediaPlayer == null)
            return;
    ...
    static int getSelectedTrack(IMediaPlayer mp, int trackType) {
        IjkMediaPlayer ijkMediaPlayer = getIjkMediaPlayer(mp);
        if (ijkMediaPlayer == null)
            return -1;
